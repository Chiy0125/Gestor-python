...
class CenterWidgetMixin:
    def center(self):
        self.update()
        w = self.winfo_width() #Ancho de la pantalla automaticamente
        h = self.winfo_height() #Altura de la pantalla automaticamente
        ws = self.winfo_screenwidth() #tamaño del monitor
        hs = self.winfo_screenheight() #para transformar los calculos en enteros
        x = int(ws/2 - w/2)
        y = int(hs/2 - h/2)
        self.geometry(f"{w}x{h}+{x}+{y}") #self.geometry("WIDTHxHEIGTH+OFFSET_X+OFFSET_Y")
